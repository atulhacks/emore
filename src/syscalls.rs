/*─────────────────────────────────────────────────────────────────────────────*\
 | Indirect Syscalls Module for Emore
 |
 | Bypasses usermode API hooks by directly invoking syscall stubs from ntdll.dll
 | Extracts SSNs and calls syscalls without triggering EDR/AV hooks on Win32 APIs
\*─────────────────────────────────────────────────────────────────────────────*/
...
fn rva_to_ptr(
    ...
    rva: usize,
    image_size: usize,
    _sections: &[IMAGE_SECTION_HEADER],
    ctx: &str,
...
fn init_syscalls() -> Result<(*const u8, usize), String> {
    ...
    if SYSCALL_TABLE.get().is_some() {
        // Already initialized, just get ntdll base again
        unsafe {
            #[repr(C)]
            ...
            Err("ntdll.dll not found in loaded module list".into())
        }
    } else {
...
pub fn indirect_syscall(syscall_name: &str, args: &[u64]) -> Result<u64, String> {
    ...
    // Silent operation - no debug output in release builds
    #[cfg(debug_assertions)]
    println!("indirect_syscall('{}') → NTSTATUS 0x{:08X} (ssn: 0x{:03X})", syscall_name, ret as u32, _ssn);
